...
export const roadmap = [
  {
    ...
    topics: [
      ...
      },
      {
        ...
        detail:
          "XSS: reflected, stored, DOM-based — как возникает в React (dangerouslySetInnerHTML, небезопасные URL). CSP: default-src, script-src, зачем nonce/hash. Sanitization: DOMPurify для HTML из API. Задача: найти в проекте 2 потенциально опасных места и задокументировать митигацию в Notion.",
      },
      {
        id: "q1-fe-10",
        track: "frontend",
      ...
      // --- AI ---
      {
        id: "q1-ai-1",
        track: "ai",
        title: "Cursor: Tab и автодополнение в редакторе",
        detail:
          'Комментарии как подсказки: "// функция принимает массив users и возвращает...". Цикл: написал комментарий → принял предложение Cursor → ПРОЧИТАЛ и ПОНЯЛ что принял → улучшил. Золотое правило: никогда не принимать сгенерированный код не читая его. Настроить .cursor/rules под стиль проекта.',
      },
      {
        id: "q1-ai-2",
        track: "ai",
        title: "Cursor Chat: шаблоны запросов",
        detail:
          'Создать раздел "AI промпты" в Notion. Добавить: "объясни этот код пошагово", "найди потенциальные баги", "как рефакторить с паттерном X", "какие edge cases я упустил". Использовать @-упоминания файлов для контекста. Для быстрых вопросов без репозитория — Gemini.',
      },
      {
        id: "q1-ai-3",
        track: "ai",
        title: "Стек AI: Cursor + Gemini",
        detail:
          "Зафиксировать в Notion правила: Cursor — рефакторинг, код в проекте, Agent-задачи с файлами; Gemini — теория, сравнение подходов, черновики текстов. Практика: одну тему недели разобрать в Gemini (конспект), применить в Cursor (код). Не дублировать один вопрос в обоих без цели.",
      },
      // --- PM ---
      {
        id: "q1-pm-1",
      ...
      },
      {
        ...
        detail:
          "Взять реальную техническую задачу (например, рефакторинг модуля авторизации или внедрение a11y-аудита). Разбить: эпики (месяц) → пользовательские истории (спринт) → технические задачи (день). Оценить в story points. Это упражнение на Lead-мышление.",
      },
      {
        id: "q1-pm-3",
        track: "pm",
...

  {
    ...
    topics: [
      ...
      // --- AI ---
      {
        id: "q2-ai-1",
        track: "ai",
        title: "Cursor для архитектурных решений",
        detail:
          'Промпт с @файлами: "Проектирую страницу с этими требованиями [описание]. Предложи разбивку на Server/Client компоненты с обоснованием." Критически оценивать: AI не знает контекст бизнеса. Ответ — отправная точка, не готовое решение.',
      },
      {
        id: "q2-ai-2",
        track: "ai",
        ...
        detail:
          "Сгенерировать через Cursor тест для компонента. Проверить: покрывает ли edge cases, проверяет ли accessibility (getByRole), нет ли хрупких селекторов. Добавить вручную 2-3 теста, которые AI упустил. Цель: понять как думать о покрытии.",
      },
      {
        id: "q2-ai-3",
        track: "ai",
...

  {
    ...
    topics: [
      ...
      },
      {
        ...
        detail:
          'Cursor: генерация JSDoc из кода. Промпт: "Сгенерируй README для этого компонента [код]: props table, usage examples, accessibility notes, known limitations." Всегда проверять и дополнять вручную — особенно a11y и security.',
      },
      // --- PM ---
      {
        id: "q3-pm-1",
...

  {
    ...
    topics: [
      ...
      },
      {
        ...
        detail:
          'Чеклист ревьюера: correctness, performance (нет лишних ре-рендеров), security (XSS, секреты, auth), readability, tests, accessibility (семантика, keyboard, ARIA). Правило: не "это неправильно", а "предлагаю X, потому что Y". Написать "Code Review Guidelines" — отдельные секции Security и A11y.',
      },
      {
        id: "q4-fe-8",
        track: "frontend",
      ...
      },
      {
        ...
        detail:
          "Написать политику (1 страница в Notion): Cursor — boilerplate, тесты, рефакторинг с ревью; Gemini — обучение и черновики без прод-кода. Обязательное ревью: архитектура, бизнес-логика, security, auth, payment. Запрет: слепое копирование без понимания. Часть DX Manual.",
      },
      // --- PM ---
      {
        id: "q4-pm-1",
